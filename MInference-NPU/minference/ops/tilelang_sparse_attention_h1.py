...
def build_sparse_attention_mh_block_index_fwd(
    *,
    ...
    """Multi-head block-sparse attention via fold-into-batch: reuses H=1 kernel.

    The caller pre-folds all heads into the batch dimension before calling so
    the kernel always sees contiguous single-head slices (stride_S = D).

    Root cause of the earlier NaN: ``T.copy(Q[b, s:s+BM, h, :D])`` on
    ``[B, S, H, D]`` has stride_S = H*D (non-contiguous for H > 1).
    Folding H into batch gives ``[B*H, S, 1, D]`` with stride_S = D. ✓

    Callable signature: ``kernel(q, k, v, block_indices) -> out``.
    Shapes (caller must pre-fold):
      * ``q``:             ``[B*H, S_q, 1, D]``
      * ``k``, ``v``:      ``[B*H, S_k, 1, D]``
      * ``block_indices``: ``[B*H, n_q_blocks, 1, topk_blocks]``
    """
    return build_sparse_attention_h1_block_index_fwd(
        dim=dim,
        topk_blocks=topk_blocks,
        sm_scale=sm_scale,
        block_M=block_M,
